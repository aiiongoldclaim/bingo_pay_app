...
| `lib/features/payment/presentation/cubit/payment_state.dart` | No new persisted fields. `customerBalance` from the intent response is used transiently inside `makePayment()` for a client-side insufficient-balance pre-check before confirming (single-tap flow — no intermediate review step); `orderId` now always comes from a real `order.orderNumber`. |
...
| `lib/features/scanner/data/datasource/payment_remote_datasource.dart` | Make `processPayment()` always throw a clear "temporarily unavailable" `ServerException` instead of calling the leaked-key endpoint. This flow has no backend equivalent (freeform P2P transfer isn't a real BIGOD operation) and its only backend dependency is the key being rotated away. The existing scanner `PaymentCubit` already catches any exception and surfaces its message through the screen's existing SnackBar listener, so no screen/router changes are needed. Full replacement is Spec 2. |
