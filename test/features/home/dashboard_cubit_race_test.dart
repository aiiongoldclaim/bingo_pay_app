...
void main() {
  test('a stale loadHome() response (slow network) cannot overwrite a newer '
      'pull-to-refresh that started after it and finished first', () async {
    ...
    await secondLoad;
    expect(cubit.state.recommended.map((p) => p.uuid), [
    ...
    expect(
      ...
      ['fresh-from-second-call'],
      reason:
          'a request-id guard must drop the first call\'s late, '
