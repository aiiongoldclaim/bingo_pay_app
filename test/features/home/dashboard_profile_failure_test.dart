...
void main() {
  test('profile endpoint 500s while categories and products succeed → '
      'userName/balance silently default to \'\'/0.0, dashboard still loads, '
      'no dashboard-wide error', () async {
    ...
    expect(
      ...
      HomeStatus.loaded,
      reason:
          'products/categories succeeded, so this must stay a '
    ...
    expect(
      ...
      isNull,
      reason:
          'a profile-only failure must not surface as a visible '
          'dashboard error',
    );
