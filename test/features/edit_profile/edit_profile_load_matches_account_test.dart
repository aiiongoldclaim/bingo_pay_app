...
void main() {
  ...
  test(
    'load() shows the exact same fullName/email the Profile screen '
    '(GetProfileUseCase) returns — not a separately-fetched, '
    'possibly-out-of-sync value',
    () async {
      ...
      expect(cubit.state.profile?.fullName, _account.fullName);
      expect(cubit.state.profile?.email, _account.email);
      expect(cubit.state.profile?.phoneNumber, _account.phone);
      expect(cubit.state.profile?.profileImageUrl, _account.profileImageUrl);
    },
  );
